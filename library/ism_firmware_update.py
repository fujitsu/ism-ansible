...
class IsmFirmwareUpdate():
           ...
           # ip transformation
            ...
            for update_param_hash in self.module.params['firmware_update_list']:
                ...
                if "firmware_version" in update_param_hash:
                    ...
                    chk = self.checkFirmwareVersion(common, update_param_hash['firmware_name'], update_param_hash['firmware_version'])
    ...
    def checkFirmwareVersion(self, common, firmware_name, firmware_version):
        try :
            self.module.debug("***** checkFirmwareVersion Start *****")
            ...
            if "Firmware" in json_data["IsmBody"]["Node"]["VariableData"]:
                if json_data["IsmBody"]["Node"]["VariableData"]["Firmware"] is not None:
                    for list in json_data["IsmBody"]["Node"]["VariableData"]["Firmware"]:
                        if (list["Name"] == firmware_name) and (list["FirmwareVersion"] == firmware_version):
                            result = False
                            break
                            
            self.module.debug("***** checkFirmwareVersion End *****")
    ...
    def taskCheck(self, id, common):
        try :
            ...
            time_out = long(unix_time) + long(FIRMWARE_UPDATE_TIME_OUT)
            ...
            while True:
                ...
                if long(now_time) < long(time_out):
                    ...
                    self.module.log("waiting for task timeout: " + str(FIRMWARE_UPDATE_TIME_OUT) + "s")
                    self.module.fail_json(msg="waiting for task timeout: " + str(FIRMWARE_UPDATE_TIME_OUT) + "s")
                    break
